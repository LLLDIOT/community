...
| M2 | 简历上传 + 投递 + 状态机 API | ✅ |
| M3 | 前端框架 + 社团资料与招新管理页 | ✅ |
| M4 | 前端简历库：列表/详情/筛选/归档树 | ✅ |
| M5 | 认证与角色权限（JWT） | ⏸ 暂缓 —— 当前为内部开放模式，上线前必须启用 |
| M6 | 导出/统计/Docker/部署文档 | ✅（导出、Docker、部署文档；统计依赖 M5 后的账号维度，暂缓） |

> **M5 暂缓说明**：系统按社团管理内部工具设计，一期先以"开放模式"交付。
> 正式对外/多社团使用时，需补：user 表 + JWT 登录 + club_admin 与 club 绑定 + 路由守卫（后端 role 中间件 + 前端登录页与拦截器），设计已预留（JWT payload `{ sub, role }`、角色守卫位置标注在 service 的 `assertClubAccess` TODO 处）。
